...
// Path to colony files for turtle download
const COLONY_ROOT = path.join(__dirname, '..', '..');
...
const server = http.createServer((req, res) => {
    ...
    // Serve colony Lua files for turtles to download
    if (url.pathname.startsWith('/colony/')) {
        const luaPath = path.join(COLONY_ROOT, url.pathname.replace('/colony/', ''));
        
        fs.readFile(luaPath, 'utf8', (err, content) => {
            if (err) {
                res.writeHead(404);
    ...
    if (url.pathname === '/files') {
        ...
        function scanDir(dir, prefix) {
            try {
                ...
                for (const item of items) {
                    ...
                    if (stat.isDirectory() && item !== 'dashboard' && item !== 'node_modules') {
        ...
        res.end(`
            <h1>Colony Files</h1>
            <p>Download these on your turtle using wget:</p>
            <pre>${files.map(f => `wget http://YOUR_IP:3000/colony/${f} /${f}`).join('\n')}</pre>
            <h2>Or run the installer:</h2>
            <pre>wget run http://YOUR_IP:3000/colony/install-http.lua</pre>
        `);
        return;
    }
