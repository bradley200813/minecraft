...
function I.refuelFromLava()
    ...
    -- Helper to check if we got lava
    local function checkLava()
        local it = turtle.getItemDetail()
        return it and it.name == "minecraft:lava_bucket"
    end
    
    -- Method 1: Try attack (left-click) - Functional Storage Fluid Drawers need this!
    if not gotLava then
        turtle.attack()
        if checkLava() then gotLava = true end
    end
    if not gotLava then
        turtle.attackUp()
        if checkLava() then gotLava = true end
    ...
    -- Method 2: Try place (right-click) - vanilla lava sources
    if not gotLava then
        turtle.place()
        if checkLava() then gotLava = true end
    end
    ...
    -- Method 3: Try drop/suck for some fluid containers
    if not gotLava then
        for _, info in ipairs({
            {drop = turtle.drop, suck = turtle.suck},
            {drop = turtle.dropUp, suck = turtle.suckUp},
            {drop = turtle.dropDown, suck = turtle.suckDown}
        }) do
            turtle.select(bucketSlot)
            if info.drop(1) then
                sleep(0.1)
    ...
    turtle.select(1)
    return false, "No lava (need bucket + face fluid drawer)"
end
